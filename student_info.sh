...
echo "$($PSQL "SELECT course FROM majors INNER JOIN students USING(major_id) INNER JOIN majors_courses USING(major_id) INNER JOIN courses USING(course_id) GROUP BY course HAVING COUNT(student_id)=1 ORDER BY course;")"
